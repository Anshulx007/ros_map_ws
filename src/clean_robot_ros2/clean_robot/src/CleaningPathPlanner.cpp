...
CleaningPathPlanning::CleaningPathPlanning(nav2_costmap_2d::Costmap2DROS *costmap2d_ros)
{
    ...
    auto private_nh = std::make_shared<rclcpp::Node>("cleaning_plan_nodehandle");
    plan_pub_ = private_nh->create_publisher<nav_msgs::msg::Path>("cleaning_path", 1);
    grid_pub_ = private_nh->create_publisher<nav_msgs::msg::OccupancyGrid>("covered_grid", 1);

    SIZE_OF_CELL = 3;
    private_nh->declare_parameter("size_of_cell", 3);
    private_nh->get_parameter("size_of_cell", SIZE_OF_CELL);

    GRID_COVERED_VALUE = 0;
    private_nh->declare_parameter("grid_covered_value", 0);
    private_nh->get_parameter("grid_covered_value", GRID_COVERED_VALUE);
...
void CleaningPathPlanning::GetBorderTrackingPathInCV(vector<cv::Point2i> &resultVec)
{
    ...
    for (r = 0; r < srcMap_.rows; r++)
    {
        ...
        {
            //RCLCPP_INFO(rclcpp::get_logger("clean_robot"), "The value at row %d, col %d is %d",r,c,srcMap_.at<uchar>(r,c));
            if (srcMap_.at<uchar>(r, c) == nav2_costmap_2d::INSCRIBED_INFLATED_OBSTACLE)
            {
                RCLCPP_INFO(rclcpp::get_logger("clean_robot"), "entered once!");
...
bool CleaningPathPlanning::initializeCoveredGrid() //在这里我对CoverGrid的理解为覆盖栅格。
{
    ...
    covered_path_grid_.header.stamp = rclcpp::Clock().now();
